...
#include <src/plugins/types/IntAPIValue.hpp>
#undef private
...
static SP<Plugin::IAPIValue> customValue;
...
static SDispatchResult test(std::string in) {
    ...
    if (Config::Legacy::mgr()->m_configValueNumber != Config::Supplementary::CONFIG_OPTIONS.size() + 1 /* autogenerated is special */) {
        errors += std::format("config value number mismatches descriptions size (legacy) → {} != {}\n", Config::Legacy::mgr()->m_configValueNumber,
                              Config::Supplementary::CONFIG_OPTIONS.size() + 1);
        success = false;
    }

    if (Config::Lua::mgr()->m_configValues.size() != Config::Supplementary::CONFIG_OPTIONS.size() + 1 /* autogenerated is special */) {
        errors += std::format("config value number mismatches descriptions size (lua) → {} != {}\n", Config::Lua::mgr()->m_configValues.size(),
                              Config::Supplementary::CONFIG_OPTIONS.size() + 1);
        success = false;

        static auto nameToLua = [](const std::string& v) -> std::string {
            auto cpy = v;
            std::ranges::replace(cpy, ':', '.');
            return cpy;
        };

        // for lua, we can actually find the offenders
        bool printedMissing = false, printedStray = false;

        // missing
        for (const auto& desc : Config::Supplementary::CONFIG_OPTIONS) {
            const auto NAME = nameToLua(desc.value);
            if (std::ranges::find_if(Config::Lua::mgr()->m_configValues, [&NAME](const auto& kv) { return NAME == kv.first; }) != Config::Lua::mgr()->m_configValues.end()) {
                // this value exists in our manager, good.
                continue;
            }
        ...
        // stray
        for (const auto& [k, v] : Config::Lua::mgr()->m_configValues) {
            if (std::ranges::find_if(Config::Supplementary::CONFIG_OPTIONS, [&k](const Config::Supplementary::SConfigOptionDescription& desc) {
                    const auto NAME = nameToLua(desc.value);
                    return NAME == k;
                }) != Config::Supplementary::CONFIG_OPTIONS.end()) {
                // this value exists in descriptions, good.
                continue;
            }
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    customValue = makeShared<Plugin::CIntAPIValue>("plugin:test:my_balls_itch", 69);
