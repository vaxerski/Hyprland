...
static bool test() {
    ...
    Tests::dispatchLua("hl.dsp.focus({ workspace = \"name:groups\" })");
    ...
    OK(Tests::dispatchLua("hl.dsp.focus({ window = \"class:kittyB\" })"));
    OK(Tests::dispatchLua("hl.dsp.group.toggle()"));
    OK(Tests::dispatchLua("hl.dsp.focus({ window = \"class:kittyA\" })"));
    OK(Tests::dispatchLua("hl.dsp.group.toggle()"));
    ...
    OK(Tests::dispatchLua("hl.dsp.window.move({ direction = \"r\", group_aware = true })"));
    ...
    Tests::dispatchLua("hl.dsp.group.next()");
    ...
    try {
        ...
        OK(Tests::dispatchLua("hl.dsp.group.move_window({ forward = true })"));
    ...
    try {
        auto str              = getFromSocket("/activewindow");
        auto activeBeforeMove = std::stoull(str.substr(7, str.find(" -> ") - 7), nullptr, 16);
        OK(Tests::dispatchLua("hl.dsp.group.move_window({ forward = false })"));
        str                  = getFromSocket("/activewindow");
        auto activeAfterMove = std::stoull(str.substr(7, str.find(" -> ") - 7), nullptr, 16);
        EXPECT(activeAfterMove, activeBeforeMove);
    } catch (...) {
    ...
    OK(Tests::dispatchLua("hl.dsp.focus({ direction = \"l\" })"));
    OK(Tests::dispatchLua("hl.dsp.group.active({ index = 1 })"));
    ...
    OK(Tests::dispatchLua("hl.dsp.group.active({ index = 3 })"));
    ...
    OK(Tests::evalLua("hl.config({ [\"group.groupbar.enabled\"] = false })"));
    {
        ...
        OK(Tests::dispatchLua("hl.dsp.window.move({ direction = \"d\", group_aware = true })"));
    ...
    // Test locked groups
    {
        ...
        OK(Tests::dispatchLua("hl.dsp.group.toggle()"));
        OK(Tests::dispatchLua(std::format("hl.dsp.focus({{ window = {} }})", Tests::luaQuote(std::format("pid:{}", lockedWin->pid())))));
        OK(Tests::dispatchLua("hl.dsp.group.lock_active({ action = \"enable\" })"));
