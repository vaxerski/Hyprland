...
static std::string specialWorkspace(const std::string& name) {
    return Tests::dispatchLua("hl.dsp.workspace.toggle_special(" + Tests::luaQuote(name) + ")");
}
...
static std::string focusMonitor(const std::string& selector) {
    return Tests::dispatchLua("hl.dsp.focus({ monitor = " + Tests::luaQuote(selector) + " })");
}

static std::string focusDirection(const std::string& direction) {
    return Tests::dispatchLua("hl.dsp.focus({ direction = " + Tests::luaQuote(direction) + " })");
}

static std::string moveWindowDirection(const std::string& direction) {
    return Tests::dispatchLua("hl.dsp.window.move({ direction = " + Tests::luaQuote(direction) + " })");
}
...
static void testDynamicWsEffects() {
    ...
    OK(Tests::evalLua("hl.workspace_rule({ workspace = \"69\", border_size = 20, no_rounding = true })"));
...
static bool test() {
    ...
    OK(Tests::dispatchLua("hl.dsp.window.resize({ x = 20, y = 20, relative = true })"));
