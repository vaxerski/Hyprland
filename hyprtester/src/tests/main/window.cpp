...
static std::string workspace(const std::string& selector) {
    return Tests::dispatchLua("hl.dsp.focus({ workspace = " + Tests::luaQuote(selector) + " })");
}

static std::string focusWindow(const std::string& selector) {
    return Tests::dispatchLua("hl.dsp.focus({ window = " + Tests::luaQuote(selector) + " })");
}
...
static void testSwapWindow() {
    ...
    // Test swapwindow by direction
    {
        ...
        OK(Tests::dispatchLua("hl.dsp.window.swap({ direction = \"r\" })"));
    ...
    // Test swapwindow by class
    {
        ...
        OK(Tests::dispatchLua("hl.dsp.window.swap({ target = \"class:kitty_B\" })"));
        OK(focusWindow("class:kitty_B"));
    ...
    // Test swapwindow by address
    {
        ...
        OK(Tests::dispatchLua(std::format("hl.dsp.window.swap({{ target = {} }})", Tests::luaQuote(std::format("address:0x{}", addr)))));
        OK(focusWindow(std::format("address:0x{}", addr)));
    ...
    {
        OK(Tests::dispatchLua("hl.dsp.window.fullscreen({ mode = \"fullscreen\" })"));

        auto str = Tests::dispatchLua("hl.dsp.window.swap({ direction = \"l\" })");
        EXPECT_CONTAINS(str, "Can't swap fullscreen windo");
    ...
    NLog::log("{}Testing swapwindow with different workspace", Colors::YELLOW);
    {
        ...
        OK(Tests::dispatchLua("hl.dsp.window.move({ workspace = \"name:swapwindow2\", follow = false })"));
        OK(Tests::dispatchLua(std::format("hl.dsp.window.swap({{ target = {} }})", Tests::luaQuote(std::format("address:0x{}", addr)))));
        focusWindow("class:kitty_B");
...
static void testGroupRules() {
    ...
    OK(setConfig("general.border_size", "8"));
    OK(Tests::evalLua("hl.workspace_rule({ workspace = \"w[tv1]\", border_size = 0 })"));
    OK(Tests::evalLua("hl.workspace_rule({ workspace = \"f[1]\", border_size = 0 })"));
    OK(Tests::evalLua("hl.window_rule({ match = { workspace = \"w[tv1]\" }, border_size = 0 })"));
    OK(Tests::evalLua("hl.window_rule({ match = { workspace = \"f[1]\" }, border_size = 0 })"));
    ...
    OK(Tests::dispatchLua("hl.dsp.group.toggle()"));
    OK(focusWindow("class:kitty_B"));
    OK(Tests::dispatchLua("hl.dsp.window.move({ into_group = \"l\" })"));
    ...
    OK(Tests::dispatchLua("hl.dsp.group.next()"));
    ...
    OK(Tests::dispatchLua("hl.dsp.window.move({ out_of_group = \"r\" })"));
...
static bool testWindowFocusOnFullscreenConflict() {
    ...
    OK(setConfig("misc.focus_on_activate", "true"));
    ...
    // Unfullscreen on conflict
    {
        ...
        OK(focusWindow("class:kitty_A"));
        OK(Tests::dispatchLua("hl.dsp.window.fullscreen({ mode = \"fullscreen\", action = \"set\" })"));
        ...
        OK(Tests::dispatchLua("hl.dsp.window.kill({ window = \"activewindow\" })"));
    ...
    // Take over on conflict
    {
        ...
        OK(Tests::dispatchLua("hl.dsp.window.fullscreen_state({ internal = 0, client = 0 })"));
...
static void testMaximizeSize() {
    ...
    OK(Tests::dispatchLua("hl.dsp.window.fullscreen({ mode = \"maximized\" })"));
...
static void testFloatingFocusOnFullscreen() {
    ...
    EXPECT(spawnKitty("kitty_A"), true);
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"toggle\" })"));

    EXPECT(spawnKitty("kitty_B"), true);
    OK(Tests::dispatchLua("hl.dsp.window.fullscreen({ mode = \"maximized\" })"));

    OK(Tests::dispatchLua("hl.dsp.window.cycle_next()"));

    OK(Tests::evalLua("hl.plugin.test.floating_focus_on_fullscreen()"));
...
static void testGroupFallbackFocus() {
    ...
    OK(focusWindow("class:kitty_B"));
    OK(focusWindow("class:kitty_D"));
    OK(Tests::dispatchLua("hl.dsp.window.close({ window = \"activewindow\" })"));
...
static void testBringActiveToTopMouseMovement() {
    ...
    Tests::killAllWindows();
    OK(setConfig("input.follow_mouse", "2"));
    OK(setConfig("input.float_switch_override_focus", "0"));

    EXPECT(spawnKitty("a"), true);
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"enable\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.move({ x = 500, y = 300, window = \"activewindow\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.resize({ x = 400, y = 400, window = \"activewindow\" })"));
    ...
    OK(Tests::dispatchLua("hl.dsp.cursor.move({ x = 700, y = 500 })"));

    OK(focusWindow("class:a"));
    EXPECT_CONTAINS(getFromSocket("/activewindow"), "class: a");

    OK(Tests::dispatchLua("hl.dsp.window.bring_to_top()"));
    EXPECT(getTopWindow(), std::string("a"));

    OK(Tests::evalLua("hl.plugin.test.click(\"272,1\")"));
    OK(Tests::evalLua("hl.plugin.test.click(\"272,0\")"));
...
static void testInitialFloatSize() {
    ...
    Tests::killAllWindows();
    OK(Tests::evalLua("hl.window_rule({ match = { class = \"kitty\" }, float = true })"));
    OK(setConfig("input.float_switch_override_focus", "0"));
    ...
    OK(Tests::dispatchLua("hl.dsp.exec_cmd(\"[float yes]kitty\")"));
...
static bool testWindowRuleFocusOnActivate() {
    ...
    {
        OK(setConfig("misc.focus_on_activate", "true"));
        OK(Tests::evalLua("hl.window_rule({ match = { class = \"kitty_antifocus\" }, focus_on_activate = false })"));
        ...
        OK(focusWindow("class:kitty_default"));
    ...
    {
        OK(setConfig("misc.focus_on_activate", "false"));
        OK(Tests::evalLua("hl.window_rule({ match = { class = \"kitty_superfocus\" }, focus_on_activate = true })"));
...
static bool testPinnedWorkspacesValid() {
    ...
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"enable\", window = \"class:kitty\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.pin({ action = \"toggle\", window = \"class:kitty\" })"));
    ...
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"disable\", window = \"class:kitty\" })"))
...
static bool testWindowRuleWorkspaceEmpty() {
    ...
    OK(Tests::evalLua("hl.window_rule({ match = { class = \"kitty_A\" }, workspace = \"empty\" })"));
    OK(Tests::evalLua("hl.window_rule({ match = { class = \"kitty_B\" }, workspace = \"emptyn\" })"));
...
static bool testContentRules() {
    ...
    OK(Tests::evalLua("hl.window_rule({ match = { class = \"kitty_content_string\" }, content = \"game\" })"));
    OK(Tests::evalLua("hl.window_rule({ match = { class = \"kitty_content_numbers\" }, content = 3 })"));
    OK(Tests::evalLua("hl.window_rule({ match = { content = \"game\" }, border_size = 10 })"));
    OK(Tests::evalLua("hl.window_rule({ match = { content = 3 }, opacity = \"0.5\" })"));
...
static bool test14038() {
    ...
    OK(Tests::dispatchLua("hl.dsp.window.move({ workspace = \"special:a\", follow = false, window = \"class:kitty_14038\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"toggle\", window = \"class:kitty_14038\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.pin({ action = \"toggle\", window = \"class:kitty_14038\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"toggle\", window = \"class:kitty_14038\" })"));
...
static bool test() {
    ...
    NLog::log("{}Testing window split ratios", Colors::YELLOW);
    {
        ...
        OK(setConfig("dwindle.default_split_ratio", "1.25"));
        ...
        OK(setConfig("dwindle.default_split_ratio", "0.75"));
        ...
        OK(setConfig("dwindle.default_split_ratio", "1"));
    }

    // open xeyes
    NLog::log("{}Spawning xeyes", Colors::YELLOW);
    Tests::dispatchLua("hl.dsp.exec_cmd(\"xeyes\")");
    ...
    // check some window props of xeyes, try to float it
    {
        ...
        Tests::dispatchLua(R"(hl.dsp.window.float({ action = "enable", window = "class:XEyes" }))");
    ...
    NLog::log("{}Testing spawning a floating window over a fullscreen window", Colors::YELLOW);
    {
        ...
        OK(Tests::dispatchLua("hl.dsp.exec_cmd(\"[float] kitty\")"));
        Tests::waitUntilWindowsN(2);

        OK(focusWindow("class:^kitty$"));
    ...
    NLog::log("{}Testing minsize/maxsize rules for tiled windows", Colors::YELLOW);
    {
        ...
        OK(setConfig("misc.size_limits_tiled", "true"));
        OK(Tests::evalLua("hl.window_rule({ name = \"kitty-max-rule\", match = { class = \"kitty_maxsize\" }, max_size = { 1500, 500 }, min_size = { 1200, 500 } })"));
        ...
        OK(focusWindow("class:kitty_maxsize"));
        //        EXPECT_CONTAINS(getFromSocket("/activewindow"), "size: 1200,500")
    ...
    NLog::log("{}Testing minsize/maxsize rules", Colors::YELLOW);
    {
        ...
        OK(setConfig("misc.size_limits_tiled", "false"));
        OK(Tests::evalLua("hl.window_rule({ name = \"kitty-max-rule\", match = { class = \"kitty_maxsize\" }, max_size = { 1500, 500 }, min_size = { 1200, 500 } })"));
        if (!spawnKitty("kitty_maxsize"))
            return false;
...

    {
        // Set float
        OK(Tests::evalLua(
            "hl.window_rule({ name = \"kitty-max-rule\", match = { class = \"kitty_maxsize\" }, max_size = { 1200, 500 }, min_size = { 1200, 500 }, float = true })"));
        if (!spawnKitty("kitty_maxsize"))
            return false;
    ...
    OK(Tests::evalLua("hl.window_rule({ name = \"wr-kitty-stuff\", opacity = \"0.5 0.5 override\" })"));
    ...
    OK(Tests::evalLua("hl.window_rule({ name = \"special-magic-kitty\", match = { class = \"magic_kitty\" }, workspace = \"special:magic\" })"));
    ...
    if (auto str = getFromSocket("/monitors"); str.contains("magic)")) {
        OK(Tests::dispatchLua("hl.dsp.workspace.toggle_special(\"magic\")"));
    }

    Tests::killAllWindows();

    OK(Tests::evalLua("hl.window_rule({ name = \"border-magic-kitty\", match = { class = \"border_kitty\" }, border_color = { colors = { \"rgba(c6ff00ff)\", \"rgba(ff0000ee)\" }, "
                      "angle = 45 } })"));

    if (!spawnKitty("border_kitty"))
        return false;

    OK(focusWindow("class:border_kitty"));
    ...
    OK(Tests::evalLua("hl.window_rule({ match = { class = \"overlap_kitty\" }, border_size = 0 })"));
    OK(Tests::evalLua("hl.window_rule({ match = { fullscreen = false }, border_size = 10 })"));
    ...
    OK(Tests::evalLua("hl.window_rule({ match = { class = \"persistent_size_kitty\" }, persistent_size = true, float = true })"));

    if (!spawnKitty("persistent_size_kitty"))
        return false;

    OK(Tests::dispatchLua("hl.dsp.window.resize({ x = 600, y = 400 })"))
    ...
    OK(Tests::evalLua("hl.window_rule({ match = { float = true }, border_size = 10 })"));
    ...
    OK(Tests::evalLua("hl.window_rule({ match = { class = \"expr_kitty\" }, float = true, size = \"monitor_w*0.5 monitor_h*0.5\", min_size = \"monitor_w*0.25 monitor_h*0.25\", "
    ...
    OK(Tests::evalLua("hl.plugin.test.add_window_rule()"));
    OK(getFromSocket("/reload"));

    OK(Tests::evalLua("hl.window_rule({ match = { class = \"plugin_kitty\" }, plugin_rule = \"effect\" })"));

    if (!spawnKitty("plugin_kitty"))
        return false;

    OK(Tests::evalLua("hl.plugin.test.check_window_rule()"));
    ...
    OK(Tests::evalLua("hl.window_rule({ name = \"test-plugin-rule\", match = { class = \"plugin_kitty\" }, plugin_rule = \"effect\" })"));
