...
static void testSwapWindow() {
    ...
    // Test swapwindow by direction
    {
        ...
        OK(Tests::dispatchLua("hl.window.swap({ direction = \"r\" })"));
    ...
    // Test swapwindow by class
    {
        ...
        OK(Tests::dispatchLua("hl.window.swap({ target = \"class:kitty_B\" })"));
        OK(focusWindow("class:kitty_B"));
    ...
    // Test swapwindow by address
    {
        ...
        OK(Tests::dispatchLua(std::format("hl.window.swap({{ target = {} }})", Tests::luaQuote(std::format("address:0x{}", addr)))));
    ...
    NLog::log("{}Testing swapwindow with fullscreen. Expecting to fail", Colors::YELLOW);
    {
        ...
        auto str = Tests::dispatchLua("hl.window.swap({ direction = \"l\" })");
    ...
    NLog::log("{}Testing swapwindow with different workspace", Colors::YELLOW);
    {
        ...
        OK(Tests::dispatchLua("hl.window.move({ workspace = \"name:swapwindow2\", follow = false })"));
        OK(Tests::dispatchLua(std::format("hl.window.swap({{ target = {} }})", Tests::luaQuote(std::format("address:0x{}", addr)))));
        focusWindow("class:kitty_B");
...
static void testGroupRules() {
    ...
    OK(Tests::dispatchLua("hl.window.move({ out_of_group = \"r\" })"));
...
static bool testWindowFocusOnFullscreenConflict() {
    ...
    // Unfullscreen on conflict
    {
        ...
        OK(focusWindow("class:kitty_A"));
        OK(Tests::dispatchLua("hl.window.fullscreen({ mode = \"fullscreen\", action = \"set\" })"));
    ...
    // Take over on conflict
    {
        ...
        OK(Tests::dispatchLua("hl.window.fullscreen_state({ internal = 0, client = 0 })"));
...
static void testFloatingFocusOnFullscreen() {
    ...
    EXPECT(spawnKitty("kitty_A"), true);
    OK(Tests::dispatchLua("hl.window.float({ action = \"toggle\" })"));

    EXPECT(spawnKitty("kitty_B"), true);
    OK(Tests::dispatchLua("hl.window.fullscreen({ mode = \"maximized\" })"));

    OK(Tests::dispatchLua("hl.window.cycle_next()"));
...
static void testGroupFallbackFocus() {
    ...
    OK(Tests::dispatchLua("hl.window.close({ window = \"activewindow\" })"));
...
static void testBringActiveToTopMouseMovement() {
    ...
    OK(Tests::dispatchLua("hl.window.float({ action = \"enable\" })"));
    OK(Tests::dispatchLua("hl.window.move({ x = 500, y = 300, window = \"activewindow\" })"));
    OK(Tests::dispatchLua("hl.window.resize({ x = 400, y = 400, window = \"activewindow\" })"));
    ...
    OK(Tests::dispatchLua("hl.cursor.move({ x = 700, y = 500 })"));

    OK(focusWindow("class:a"));
    EXPECT_CONTAINS(getFromSocket("/activewindow"), "class: a");

    OK(Tests::dispatchLua("hl.window.bring_to_top()"));
...
static void testInitialFloatSize() {
    ...
    OK(Tests::dispatchLua("hl.exec_cmd(\"[float yes]kitty\")"));
...
static bool testPinnedWorkspacesValid() {
    ...
    OK(Tests::dispatchLua("hl.window.pin({ action = \"toggle\", window = \"class:kitty\" })"));
    ...
    OK(Tests::dispatchLua("hl.window.float({ action = \"disable\", window = \"class:kitty\" })"))
...
static bool test14038() {
    ...
    OK(Tests::dispatchLua("hl.window.move({ workspace = \"special:a\", follow = false, window = \"class:kitty_14038\" })"));
    OK(Tests::dispatchLua("hl.window.float({ action = \"toggle\", window = \"class:kitty_14038\" })"));
    OK(Tests::dispatchLua("hl.window.pin({ action = \"toggle\", window = \"class:kitty_14038\" })"));
    OK(Tests::dispatchLua("hl.window.float({ action = \"toggle\", window = \"class:kitty_14038\" })"));
...
static bool test() {
    ...
    Tests::dispatchLua("hl.exec_cmd(\"xeyes\")");
    ...
    // check some window props of xeyes, try to float it
    {
        ...
        Tests::dispatchLua(R"(hl.window.float({ action = "enable", window = "class:XEyes" }))");
    ...
    NLog::log("{}Testing spawning a floating window over a fullscreen window", Colors::YELLOW);
    {
        ...
        OK(Tests::dispatchLua("hl.exec_cmd(\"[float] kitty\")"));
        Tests::waitUntilWindowsN(2);
    ...
    NLog::log("{}Testing minsize/maxsize rules for tiled windows", Colors::YELLOW);
    {
        ...
        OK(setConfig("misc.size_limits_tiled", "1"));
    ...
    NLog::log("{}Testing minsize/maxsize rules", Colors::YELLOW);
    {
        ...
        OK(setConfig("misc.size_limits_tiled", "0"));
        OK(Tests::evalLua("hl.window_rule({ name = \"kitty-max-rule\", match = { class = \"kitty_maxsize\" }, max_size = { 1500, 500 }, min_size = { 1200, 500 } })"));
        if (!spawnKitty("kitty_maxsize"))
            return false;
    ...
    if (auto str = getFromSocket("/monitors"); str.contains("magic)")) {
        OK(Tests::dispatchLua("hl.workspace({ special = \"magic\" })"));
    }
    ...
    OK(Tests::dispatchLua("hl.window.resize({ x = 600, y = 400 })"))
