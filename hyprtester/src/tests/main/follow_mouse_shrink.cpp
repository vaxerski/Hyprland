...
static std::string moveCursor(const int x, const int y) {
    return Tests::dispatchLua("hl.dsp.cursor.move({ x = " + std::to_string(x) + ", y = " + std::to_string(y) + " })");
}
...
static bool test() {
    ...
    OK(Tests::dispatchLua("hl.dsp.focus({ workspace = \"name:follow_mouse_shrink\" })"));
    ...
    OK(setConfig("input.float_switch_override_focus", "2"));

    // Spawn two floating windows with a 20px gap
    // fms_a: position (100,100), size 400x400 -> hitbox [100,499] x [100,499]
    if (!spawnKitty("fms_a"))
        return false;
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"enable\", window = \"activewindow\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.resize({ x = 400, y = 400, window = \"activewindow\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.move({ x = 100, y = 100, window = \"activewindow\" })"));
    ...
    OK(setConfig("input.follow_mouse_shrink", "20"));
    // Focus A explicitly
    OK(focusWindow("class:fms_a"));
    EXPECT(waitForActiveWindow("fms_a"), true);
    OK(moveCursor(300, 300));
