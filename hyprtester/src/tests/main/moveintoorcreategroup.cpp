...
static bool test() {
    ...
    Tests::dispatchLua("hl.workspace(\"name:moveintoorcreategroup\")");
    ...
    OK(Tests::dispatchLua("hl.window.move({ into_or_create_group = \"r\" })"));
    ...
    OK(Tests::dispatchLua("hl.focus({ window = \"class:kitty_D\" })"));
    OK(Tests::dispatchLua("hl.group.toggle()"));

    OK(Tests::dispatchLua("hl.focus({ window = \"class:kitty_E\" })"));

    NLog::log("{}Move kittyE into existing group with kittyD", Colors::YELLOW);
    OK(Tests::dispatchLua("hl.window.move({ into_or_create_group = \"l\" })"));
