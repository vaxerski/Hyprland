...
static bool test() {
    ...
    Tests::spawnKitty();
    OK(Tests::dispatchLua("hl.cursor.move_to_corner({ corner = 0 })"));
    ...
    // Test that `workspace previous` works correctly after a workspace gesture.
    {
        ...
        OK(Tests::dispatchLua("hl.workspace(3)"));

        // Come to workspace 5 from workspace 3: 5 will remember that.
        OK(Tests::dispatchLua("hl.workspace(5)"));
        ...
        OK(Tests::dispatchLua("hl.workspace(\"previous\")"));
