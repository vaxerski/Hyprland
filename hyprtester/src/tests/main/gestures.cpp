...
static bool test() {
    ...
    Tests::dispatchLua("hl.dsp.focus({ workspace = 1 })"); // no OK: we might be on 1 already
    ...
    OK(Tests::evalLua("hl.plugin.test.gesture(\"up,5\")"));
    OK(Tests::evalLua("hl.plugin.test.gesture(\"down,5\")"));
    OK(Tests::evalLua("hl.plugin.test.gesture(\"left,5\")"));
    OK(Tests::evalLua("hl.plugin.test.gesture(\"right,5\")"));
    OK(Tests::evalLua("hl.plugin.test.gesture(\"right,4\")"));

    EXPECT(waitForWindowCount(0, "Gesture sent paste exit + enter to kitty"), true);

    EXPECT(Tests::windowCount(), 0);

    OK(Tests::evalLua("hl.plugin.test.gesture(\"left,3\")"));

    EXPECT(waitForWindowCount(1, "Gesture spawned kitty"), true);

    EXPECT(Tests::windowCount(), 1);

    OK(Tests::evalLua("hl.plugin.test.gesture(\"right,3\")"));
    ...
    OK(Tests::evalLua("hl.plugin.test.gesture(\"down,3\")"));
    ...
    OK(Tests::evalLua("hl.plugin.test.alt(1)"));
    ...
    OK(Tests::evalLua("hl.config({ [\"gestures.workspace_swipe_invert\"] = false })"));
    ...
    OK(Tests::evalLua("hl.config({ [\"gestures.workspace_swipe_invert\"] = true })"));
    OK(Tests::evalLua("hl.config({ [\"gestures.workspace_swipe_create_new\"] = false })"));
    ...
    OK(Tests::evalLua("hl.plugin.test.alt(0)"));

    OK(Tests::evalLua("hl.plugin.test.gesture(\"up,3\")"));
    ...
    Tests::spawnKitty();
    OK(Tests::dispatchLua("hl.dsp.cursor.move_to_corner({ corner = 0 })"));
    const std::string cursorPos1 = getFromSocket("/cursorpos");
    OK(Tests::evalLua("hl.plugin.test.gesture(\"left,4\")"));
    ...
    {
        OK(Tests::evalLua("hl.config({ [\"gestures.workspace_swipe_invert\"] = false })"));
        OK(Tests::evalLua("hl.config({ [\"gestures.workspace_swipe_create_new\"] = true })"));
        OK(Tests::dispatchLua("hl.dsp.focus({ workspace = 3 })"));

        // Come to workspace 5 from workspace 3: 5 will remember that.
        OK(Tests::dispatchLua("hl.dsp.focus({ workspace = 5 })"));
        ...
        OK(Tests::dispatchLua("hl.dsp.focus({ workspace = \"previous\" })"));
