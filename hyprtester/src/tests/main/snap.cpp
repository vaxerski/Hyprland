...
static bool spawnFloatingKitty() {
    ...
    OK(Tests::dispatchLua("hl.dsp.window.resize({ x = 100, y = 100 })"));
    return true;
}
...
static void expectSnapMove(const Vector2D FROM, const Vector2D* TO) {
    ...
    expectOk(std::format("hl.dsp.window.move({{ x = {}, y = {} }})", A.x, A.y), Tests::dispatchLua(std::format("hl.dsp.window.move({{ x = {}, y = {} }})", A.x, A.y)));
...
static bool test() {
    ...
    OK(Tests::dispatchLua("hl.dsp.focus({ monitor = \"HEADLESS-2\" })"));
    ...
    OK(Tests::dispatchLua("hl.dsp.focus({ workspace = \"name:snap\" })"));
    ...
    OK(Tests::dispatchLua("hl.dsp.window.move({ x = 500, y = 500 })"));
