...
static std::string dispatchSetProp(const std::string& selector, const std::string& prop, const std::string& value) {
    return Tests::dispatchLua("hl.window.set_prop({ window = " + Tests::luaQuote(selector) + ", prop = " + Tests::luaQuote(prop) + ", value = " + Tests::luaQuote(value) + " })");
}
...
static bool testDevicesActiveLayoutIndex() {
    ...
    // configure layouts
    Tests::evalLua("hl.config({ [\"input.kb_layout\"] = \"us,pl,ua\" })");
...
static bool testGetprop() {
    ...
    Tests::dispatchLua("hl.window.float({ action = \"enable\", window = \"class:kitty\" })"); // need to set floating for tests below
    ...
    Tests::dispatchLua("hl.window.float({ action = \"disable\", window = \"class:kitty\" })"); // go back to tiled for consistency
