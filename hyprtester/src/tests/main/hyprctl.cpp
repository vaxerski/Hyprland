...
static std::string dispatchSetProp(const std::string& selector, const std::string& prop, const std::string& value) {
    return Tests::dispatchLua("hl.dsp.window.set_prop({ window = " + Tests::luaQuote(selector) + ", prop = " + Tests::luaQuote(prop) + ", value = " + Tests::luaQuote(value) +
                              " })");
}
...
static bool testDevicesActiveLayoutIndex() {
    ...
    // configure layouts
    EXPECT(Tests::evalLua("hl.config({ [\"input.kb_layout\"] = \"us,pl,ua\" })"), "ok");

    for (uint8_t i = 0; i < 3; i++) {
        // set layout
        const auto& switchRes = getFromSocket("/switchxkblayout all " + std::to_string(i));
        if (switchRes != "ok") {
            if (switchRes.contains("layout idx out of range")) {
                NLog::log("{}Skipping higher XKB layout indices (device reports a single layout): {}", Colors::YELLOW, switchRes);
                EXPECT_CONTAINS(getFromSocket("j/devices"), R"("active_layout_index": 0)");
                return true;
            }
        ...
        // check layout index
        EXPECT_CONTAINS(devicesJson, R"("active_layout_index": )" + std::to_string(i));
    }
...
static bool testGetprop() {
    ...
    dispatchSetProp("class:kitty", "animation", "teststyle");
    ...
    dispatchSetProp("class:kitty", "max_size", "200 150");
    ...
    dispatchSetProp("class:kitty", "min_size", "100 50");
    ...
    Tests::dispatchLua(R"(hl.dsp.window.float({ action = "enable", window = "class:kitty" }))"); // need to set floating for tests below
    dispatchSetProp("class:kitty", "max_size", "90+10 25*2");                                    // set max to the same as min above, forcing window to 100*50
    ...
    dispatchSetProp("class:kitty", "min_size", "window_w*0.5 window_h-10");
    ...
    Tests::dispatchLua(R"(hl.dsp.window.float({ action = "disable", window = "class:kitty" }))"); // go back to tiled for consistency
    ...
    dispatchSetProp("class:kitty", "opacity", "0.3");
    ...
    dispatchSetProp("class:kitty", "opacity_inactive", "0.5");
    ...
    dispatchSetProp("class:kitty", "opacity_fullscreen", "0.75");
    ...
    dispatchSetProp("class:kitty", "opacity_override", "true");
    ...
    dispatchSetProp("class:kitty", "opacity_inactive_override", "true");
    ...
    dispatchSetProp("class:kitty", "opacity_fullscreen_override", "true");
    ...
    dispatchSetProp("class:kitty", "active_border_color", "rgb(abcdef)");
    ...
    dispatchSetProp("class:kitty", "allows_input", "true");
    ...
    dispatchSetProp("class:kitty", "rounding", "4");
    ...
    dispatchSetProp("class:kitty", "rounding_power", "1.25");
