...
static void testFloatClamp() {
    ...
    OK(Tests::dispatchLua("hl.dsp.focus({ window = \"class:c\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"enable\", window = \"class:c\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.resize({ x = 1200, y = 900, window = \"class:c\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"disable\", window = \"class:c\" })"));
    OK(Tests::dispatchLua("hl.dsp.window.float({ action = \"enable\", window = \"class:c\" })"));
...
static void test13349() {
    ...
    OK(Tests::dispatchLua("hl.dsp.window.move({ direction = \"r\" })"));
...
static void testSplit() {
    ...
    EXPECT_NOT(Tests::dispatchLua("hl.dsp.layout(\"swapsplit\")"), "ok");
    EXPECT_NOT(Tests::dispatchLua("hl.dsp.layout(\"splitratio 1 exact\")"), "ok");

    Tests::spawnKitty("b");

    OK(Tests::dispatchLua("hl.dsp.focus({ window = \"class:a\" })"));
    OK(Tests::dispatchLua("hl.dsp.layout(\"splitratio -0.2\")"));
    ...
    OK(Tests::dispatchLua("hl.dsp.layout(\"splitratio 1.6 exact\")"));
    ...
    EXPECT_NOT(Tests::dispatchLua("hl.dsp.layout(\"splitratio fhne exact\")"), "ok");
    EXPECT_NOT(Tests::dispatchLua("hl.dsp.layout(\"splitratio exact\")"), "ok");
    EXPECT_NOT(Tests::dispatchLua("hl.dsp.layout(\"splitratio -....9\")"), "ok");
    EXPECT_NOT(Tests::dispatchLua("hl.dsp.layout(\"splitratio ..9\")"), "ok");
    EXPECT_NOT(Tests::dispatchLua("hl.dsp.layout(\"splitratio\")"), "ok");

    OK(Tests::dispatchLua("hl.dsp.layout(\"togglesplit\")"));
    ...
    OK(Tests::dispatchLua("hl.dsp.layout(\"swapsplit\")"));
...
static void testRotatesplit() {
    ...
    OK(Tests::dispatchLua("hl.dsp.layout(\"rotatesplit\")"));
    {
        auto str = getFromSocket("/clients");
        EXPECT_CONTAINS(str, "at: 0,0");
    ...
    OK(Tests::dispatchLua("hl.dsp.layout(\"rotatesplit 180\")"));
    {
        auto str = getFromSocket("/clients");
        EXPECT_CONTAINS(str, "at: 960,0");
        EXPECT_CONTAINS(str, "size: 960,1080");
    ...
    OK(Tests::dispatchLua("hl.dsp.layout(\"rotatesplit 270\")"));
    {
        auto str = getFromSocket("/clients");
        EXPECT_CONTAINS(str, "at: 0,540");
        EXPECT_CONTAINS(str, "size: 1920,540");
    ...
    OK(Tests::dispatchLua("hl.dsp.layout(\"rotatesplit 360\")"));
    {
        auto str = getFromSocket("/clients");
        EXPECT_CONTAINS(str, "at: 0,0");
        EXPECT_CONTAINS(str, "size: 1920,540");
    ...
    OK(Tests::dispatchLua("hl.dsp.layout(\"rotatesplit -90\")"));
    {
        auto str = getFromSocket("/clients");
        EXPECT_CONTAINS(str, "at: 0,0");
        EXPECT_CONTAINS(str, "size: 960,1080");
...
static void testForceSplitOnMoveToWorkspace() {
    OK(Tests::dispatchLua("hl.dsp.focus({ workspace = 2 })"));
    ...
    OK(setConfig("dwindle.force_split", "2"));
    OK(Tests::dispatchLua("hl.dsp.cursor.move_to_corner({ corner = 3 })")); // top left
    OK(Tests::dispatchLua("hl.dsp.window.move({ workspace = \"2\" })"));
...
static bool test() {
    ...
    Tests::dispatchLua("hl.dsp.focus({ workspace = 1 })");
    OK(getFromSocket("/reload"));
