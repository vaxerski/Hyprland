...
static void testOrientations() {
    ...
    // right
    OK(Tests::dispatchLua("hl.dsp.layout(\"orientationnext\")"));
    {
        auto str = getFromSocket("/activewindow");
...
static void focusMasterPrevious() {
    ...
    OK(Tests::dispatchLua("hl.dsp.layout(\"focusmaster master\")"));
    EXPECT_CONTAINS(getFromSocket("/activewindow"), "class: master");

    // test
    NLog::log("{}Testing fallback to focusmaster auto", Colors::YELLOW);

    OK(Tests::dispatchLua("hl.dsp.layout(\"focusmaster previous\")"));
    const std::string firstSlave = activeWindowClass();
    EXPECT(firstSlave.empty(), false);
    EXPECT(firstSlave == "master", false);

    NLog::log("{}Testing focusing from slave to master via previous", Colors::YELLOW);

    OK(Tests::dispatchLua("hl.dsp.layout(\"focusmaster previous\")"));
    EXPECT_CONTAINS(getFromSocket("/activewindow"), "class: master");

    NLog::log("{}Testing previous remembers the last non-master", Colors::YELLOW);

    OK(Tests::dispatchLua("hl.dsp.layout(\"focusmaster previous\")"));
    EXPECT_CONTAINS(getFromSocket("/activewindow"), "class: " + firstSlave);

    NLog::log("{}Testing cyclenext noloop then previous", Colors::YELLOW);

    OK(Tests::dispatchLua("hl.dsp.layout(\"cyclenext noloop\")"));
    const std::string cycleTarget = activeWindowClass();
    EXPECT(cycleTarget.empty(), false);
    EXPECT(cycleTarget == "master", false);

    OK(Tests::dispatchLua("hl.dsp.layout(\"focusmaster previous\")"));
    EXPECT_CONTAINS(getFromSocket("/activewindow"), "class: master");

    OK(Tests::dispatchLua("hl.dsp.layout(\"focusmaster previous\")"));
    EXPECT_CONTAINS(getFromSocket("/activewindow"), "class: " + cycleTarget);

    // Ensure orientation checks run with master focused.
    OK(Tests::dispatchLua("hl.dsp.layout(\"focusmaster master\")"));
    EXPECT_CONTAINS(getFromSocket("/activewindow"), "class: master");
...
static void testFsBehavior() {
    ...
    OK(Tests::dispatchLua("hl.dsp.focus({ window = \"class:master\" })"));
...
static bool test() {
    ...
    // setup
    OK(Tests::dispatchLua("hl.dsp.focus({ workspace = \"name:master\" })"));
