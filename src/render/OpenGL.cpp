...
void CHyprOpenGLImpl::end() {
    ...
    if (m_offloadedFramebuffer) {
        ...
        const auto PRIMITIVE_BLOCKED =
            m_finalScreenShader.program >= 1 || g_pHyprRenderer->m_crashingInProgress || m_renderData.pMonitor->m_imageDescription != SImageDescription{};
...
void CHyprOpenGLImpl::passCMUniforms(SShader& shader, const NColorManagement::SImageDescription& imageDescription,
    ...
    if (!targetImageDescription.icc.present) {
        ...
        targetImageDescription.icc.lutTexture->bind();

        shader.setUniformInt(SHADER_LUT_3D, 8);
        shader.setUniformFloat(SHADER_LUT_SIZE, targetImageDescription.icc.lutSize);
...
void CHyprOpenGLImpl::renderTextureInternal(SP<CTexture> tex, const CBox& box, const STextureRenderData& data) {
    ...
    const bool skipCM = !*PENABLECM || !m_cmSupported                                             /* CM unsupported or disabled */
        ...
        || (imageDescription == NColorManagement::SImageDescription{} && !CANT_CHECK_CM_EQUALITY) /* Source and target have the same image description */
    ...
    if (shader == &m_shaders->m_shCM) {
        ...
        if (data.cmBackToSRGB) {
            ...
            passCMUniforms(*shader, NColorManagement::SImageDescription{});
        else
            passCMUniforms(*shader, imageDescription,
                           NColorManagement::SImageDescription{.transferFunction = CM_TRANSFER_FUNCTION_GAMMA22}); // CM into SRGB with Gamma 2.2 (akin to KDE, cuz we love KDE)
    }
